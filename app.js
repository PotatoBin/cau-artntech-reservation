...
function isAvailableTime(){
  ...
  if(day===0||day===6){console.log("[WARN] Weekend"); return false;}
  if(hour<9||hour>=22){console.log("[WARN] Out of hours"); return false;}
...
async function reserveCharger(reqBody,res,type){
  ...
  try{
    ...
    if(!isAvailableTime()){
      ...
      const desc=`평일 9시~22시 당일 예약만 가능`;
      return res.send({
        "version":"2.0",
        "template":{
          "outputs":[{"textCard":{
  ...
  try{
    ...
    try{
      ...
      if(logRow.kakao_id!==kakao_id){
        ...
        const d=`신청자의 카카오톡 계정으로 취소를 진행해주세요.`;
        res.send({
          "version":"2.0",
          "template":{
            "outputs":[{"textCard":{
...
async function reserveStartTimeCheck(reqBody,res){
  ...
  try{
    ...
    if(diff<30 && diff<0){
      ...
      return res.send({status:"FAIL",message:"30분 전 시간은 예약불가"});
    }
...
async function reserveClientInfoCheck(reqBody,res){
  ...
  try{
    ...
    if(!/^\d{11}$/.test(pho)){
      ...
      return res.send({status:"FAIL",message:"전화번호 11자리"});
    }
...
/***********************************************
 * generateReserveCode (중복 검사 최대 5회 재시도)
 ***********************************************/
async function generateReserveCode(rtype){
  ...
  while(tries<5){
    tries++;
    let code=await getNextCode(prefix);
    let used=await checkCodeExists(code);
    if(used){
      console.log("[WARN] code used->", code, "retry..");
      continue;
    }
    console.log("[INFO] code generated->", code);
    return code;
  }
  console.log("[ERROR] code gen failed after 5 tries");
  return prefix + "99999";
}
async function getNextCode(prefix){
  ...
  try{
    ...
    console.log("[DEBUG] getNextCode->",q);
...
async function checkCodeExists(code){
  const conn=await pool.getConnection();
  try{
    const q=`SELECT 1 FROM logs WHERE reserve_code=?`;
    const [rows]=await conn.execute(q,[code]);
    return rows.length>0;
  } finally{conn.release();}
}
